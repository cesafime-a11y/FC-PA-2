...
/* superficies, cancha (F/CY/GT/GB) y estado (S) ahora viven en
   core/state.js — se importan arriba.                          */
const DT=1/60;
...
/* ── estado: S ahora vive en core/state.js — se importa arriba */
...
function newMatch(){
  ...
  if(S.cfg.f7) setSuperficie(SUPERFICIES.sintetico);
...
export const other=t=>t===S.teams[0]?S.teams[1]:S.teams[0];
...
function placeFormation(t,kickTeam){
  for(const p of t.players){
    ...
    // sin esto, el render interpola desde su posición VIEJA hasta la
    // nueva y se ven deslizándose por la cancha en vez de aparecer ahí
    p.px=x;p.py=y;p.pface=p.face;
  }
}
function kickoff(kickTeam){
  ...
  taker.px=taker.x; taker.py=taker.y;
...
/* ══ REPETICIONES ═════════════════════════════════════════
   El buffer (guardar/reproducir posiciones) vive en
   src/replay/buffer.js — se importa arriba. repDibujar (la
   parte que necesita el canvas) se mudó a render.js.        */

/* ══ SONIDO ═══════════════════════════════════════════════
   Ahora vive en src/audio/sfx.js — se importa arriba.       */

/* ══ GUARDADO ═════════════════════════════════════════════
   Leer/escribir de verdad vive en src/persistence/storage.js.
   Aquí solo se decide QUÉ va en el objeto guardado — eso sí
   es del juego, no se puede desacoplar sin perder sentido.  */
...
function guardar(){
  ...
  const r=guardarLocal(localStorage,SAVE.clave,d);
  SAVE.ok=r.ok; SAVE.aviso=r.aviso;
  return r.ok;
}
function cargar(){
  const r=cargarLocal(localStorage,SAVE.clave);
  if(!r.ok){ SAVE.ok=false; SAVE.aviso=r.aviso; return false; }
  const d=r.datos;
  if(!d)return false;
...
function borrarGuardado(){ borrarLocal(localStorage,SAVE.clave); }
...
function volley(p){
  ...
  const d=dist(p,b);
  if(d>2.3||b.z>2.3)return false;   // antes 2.7m, se sentía demasiado generoso
...
function firstTouch(p){
  ...
  if(b.owner||b.frozen)return false;
  const d=dist(p,b);
  if(d>2.9||b.z>2.0)return false;   // antes 3.3m
...
export function passPlan(p,through){
  const t=p.team;
  if(isHuman(p)){
    const ap=aimPoint(p);
    ...
    // el pase normal debe llegar A LOS PIES, no al espacio de enfrente —
    // solo el filtrado (Z) se manda agresivo hacia donde va a estar el
    // compañero. Antes los dos usaban casi el mismo adelanto y por eso
    // el pase normal se sentía filtrado también.
    const lead=through?tt*1.8:tt*0.4;
...
function doPass(p,through,power,opts){
  ...
  // córner con dos estilos: el filtrado (E) ya sale arqueado/elevado por
  // la línea de arriba; si mantienes V (el mismo tiro raso de siempre)
  // justo al sacar el córner, el centro va bajo, rápido y directo en
  // vez de arqueado — un centro raso real se golpea más fuerte, no solo
  // más plano.
  if(S.restartHold&&S.restartHold.p===p&&S.restartHold.type==='corner'&&ACT.low){
    b.vz=0.12;
    b.vx*=1.25; b.vy*=1.25;
  }
  b.spin=gauss()*.5;
...
export function shotPlan(p,charge,opts){
  opts=opts||{};
  const t=p.team,b=S.ball;
...
function setPiece(type,team,pos,taker){
  ...
  if(type==='penalty'&&taker){
    ...
    taker.px=taker.x; taker.py=taker.y;
    // el arquero que defiende se coloca YA en la línea de gol, no camina
    // hasta ahí — antes se quedaba donde estuviera al momento de la falta
    // (a veces lejos, tras subir en un córner) y no le daba tiempo de volver
    const defiende=other(team);
    if(defiende.gk){
      const gk=defiende.gk;
      gk.x=defiende.dir>0?.6:F.W-.6; gk.y=CY; gk.vx=gk.vy=0;
      gk.face=Math.atan2(CY-gk.y,(team.dir>0?F.W:0)-gk.x);
      gk.px=gk.x; gk.py=gk.y;
    }
  }
  despejarDistancia();
  // si cobras tú, el control pasa a quien tira; si cobra el rival, el
  // control pasa a TU portero — antes en ese segundo caso el control se
  // quedaba en quien fuera que controlaras antes, viendo la cámara en
  // el lugar equivocado mientras el rival tiraba a tu arco
  if(!team.ai&&taker) S.ctrl=taker;
  else if(team.ai&&type==='penalty'){
    const defiende=other(team);
    if(!defiende.ai&&defiende.gk) S.ctrl=defiende.gk;
  }
}
...
function takeRestart(){
  ...
  S.restartHold={p,x:b.x,y:b.y,t:0,type:r.type};
  S.restart=null;
}
...
function possessionStep(dt){
  ...
  for(const p of S.players){
    ...
    if(p.role==='GK'){
      ...
      // antes 2.7m — con eso un portero mal colocado igual la agarraba;
      // 2.0m sigue siendo generoso (manda en su área) pero ya castiga
      // estar en el lugar equivocado
      if(Math.abs(b.x-gx)<F.SIX+1.5&&Math.abs(b.y-CY)<F.SIXW/2+1.2)reach=2.0;
    }
    const d=dist(p,b);
  ...
  if(S.restart){
    ...
    if(p===r.taker){
      ...
      if(d>.75){ steerTo(p,r.pos,true); }
        ...
        if(r.type==='penalty'){
          if(r.t>2.2&&(t.ai||r.t>4.5)) ejecutarPenal(rnd(.6,1));
          return;
        }
        if(r.t>(t.ai?.75:3.5)){
          takeRestart();
...
function userStep(dt){
  ...
  movePlayer(p,dt);
  // nadie del equipo que NO saca puede meterse a la zona reglamentaria —
  // antes esto solo lo respetaba la IA; el jugador bajo control humano
  // (tuyo o del rival) podía caminar directo a bloquear el saque
  if(S.restart && S.restart.team!==p.team && S.restart.taker!==p){
    const rd=dist(p,S.restart.pos);
    const radioMin=9.5;
    if(rd<radioMin){
      const u=rd<.4?{x:1,y:0}:norm(p.x-S.restart.pos.x,p.y-S.restart.pos.y);
      p.x=clamp(S.restart.pos.x+u.x*radioMin,1.2,F.W-1.2);
      p.y=clamp(S.restart.pos.y+u.y*radioMin,1.2,F.H-1.2);
    }
  }
...
/* ── bucle ────────────────────────────────────────────────── */
/* En un balón parado hay que SOLTAR el balón (pase o tiro) — no cargarlo
   como en juego normal. Si el que sacó sigue con él pasado un momento,
   o ya avanzó una distancia notable, se pita falta y el balón pasa al
   rival en el sitio donde se quedó. */
function restartHoldCheck(dt){
  const rh=S.restartHold; if(!rh)return;
  if(S.ball.owner!==rh.p){ S.restartHold=null; return; }   // ya lo soltó: todo bien
  rh.t+=dt;
  const avanzo=dist(rh.p,{x:rh.x,y:rh.y})>2.2;
  if(rh.t>1.4||avanzo){
    const t=rh.p.team, x=rh.p.x, y=rh.p.y;
    S.restartHold=null;
    SFX.silbato();
    freeKick(other(t),{x,y},`${t.tag}: en el saque hay que soltar el balón — se pierde por avanzar con él`);
  }
}
